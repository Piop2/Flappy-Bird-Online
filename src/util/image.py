...
def clip(surf: pygame.Surface, rect: tuple[int, int, int, int], scale: int = 1) -> pygame.Surface:
    handle_surf = surf.copy()
    clip_rect = pygame.Rect(*rect)
    handle_surf.set_clip(clip_rect)
    image = surf.subsurface(handle_surf.get_clip())
    return pygame.transform.scale_by(image.copy(), scale)


def clips(surf: pygame.Surface, rects: list[tuple[int, int, int, int]], scale: int = 1) -> list[pygame.Surface]:
    return [clip(surf, rect, scale) for rect in rects]
